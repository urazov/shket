...
$(document).ready(function(){
    ...
    /*$('#btnLoginForm').click(function(e){
        ...
        if (Login == '')
        {
            Errors.html('Не указан логин');
            return -2;
        }
        if (Pass == '')
        {
            Errors.html('Не указан пароль');
            return -2;
        }
        
        Button.html('Идет вход...');

        $.ajax({
            url: 'personalCabinet/loginInPC.php',
            type: 'POST',
            ...
            success: function(data) {  
                ...
                if(data == 1){
                    window.location.replace('personalCabinet/for_parent/PersonalCabinetForParent.php');
                } else if (data == 2){
                    window.location.replace('personalCabinet/for_teacher/PersonalCabinetForTeacher.php');                
                } else if (data == 4){
                    window.location.replace('personalCabinet/for_canteen/PersonalCabinetForCanteen.php');                
                } else if (data < 1){
                    Errors.html("Пользователь с данным логином и паролем не зарегистрирован в системе!");
                }
            },
            error: function(data) { 
                Button.html('Войти');
                Errors.html("Ошибка авторизации. Обратитесь к администратору");  
            }
        });
    ...
    $('#btnMessage').click(function(e){
        ...
        if (FIO == '')
        {
            Errors.html('Не указано ФИО');
            return -2;
        }
        if (Phone == '')
        {
            Errors.html('Не указан телефон');
            return -2;
        }
        if (Email == '')
        {
            Errors.html('Не указан Email');
            return -2;
        }
        if (Message == '')
        {
            Errors.html('Не введено сообщение');
            return -2;
        }
        ...
        $.ajax({
            url: 'SendMsg.php',
            type: 'POST',
            data: {fio : FIO, phone : Phone, email: Email, msg : Message },
            success: function(data) {
                Button.html('Отправлено');
                $('#modalMessage').modal('hide');  
                $('#modalMailto').modal('hide'); 
            },
            error: function(data) {
                Errors.html('Сервис временно недоступен. Попробуйте позже.'); 
            }
        })
    });
    ...
    $('#linkMail').click(function(e){
        ...
        var Errors = $('#mailErrors');
        Errors.html('');

        $('#modalMailto').modal({show : true});
    });
